...
test_that("merge_gtfs raises errors due to incorrect input types", {
  expect_error(merge_gtfs("spo_gtfs", poa_gtfs))
  expect_error(merge_gtfs(spo_gtfs, "poa_gtfs"))
  expect_error(merge_gtfs(spo_gtfs, poa_gtfs, files = 1))
  expect_error(merge_gtfs(spo_gtfs, poa_gtfs, quiet = "TRUE"))
  expect_error(merge_gtfs(spo_gtfs, poa_gtfs, warnings = "TRUE"))
})
...
test_that("merge_gtfs raises messages adequately", {
  expect_silent(merge_gtfs(spo_gtfs, poa_gtfs))
  expect_message(merge_gtfs(spo_gtfs, poa_gtfs, quiet = FALSE))
})

test_that("merge_gtfs results in a GTFS object", {

  # should work when files = NULL
  expect_s3_class(merge_gtfs(spo_gtfs, poa_gtfs), "dt_gtfs")
  expect_s3_class(merge_gtfs(list(spo_gtfs, poa_gtfs)), "dt_gtfs")
  ...
  expect_warning(
    merged <- merge_gtfs(spo_gtfs, poa_gtfs, files = c("shapes", "ola", "oie"))
  )
...
test_that("merge_gtfs merges the adequate 'files'", {
  ...
  poa_names <- names(poa_gtfs)
  all_names <- c(spo_names, poa_names)
  ...
  merged_gtfs_list <- merge_gtfs(list(spo_gtfs, poa_gtfs))
...
test_that("merge_gtfs bind the rows of each GTFS object adequately", {
  ...
  for (filename in merged_gtfs_names){
    ...
    expect_equal(
      ...
      nrow(rbind(spo_gtfs[[filename]], poa_gtfs[[filename]], fill = TRUE))
    )
...
test_that("merge_gtfs does not change the original GTFS objects", {
  ...
  poa_gtfs <- original_poa_gtfs <- read_gtfs(poa_path)

  merged_gtfs <- merge_gtfs(spo_gtfs, poa_gtfs)

  expect_identical(spo_gtfs, original_spo_gtfs)
  expect_identical(poa_gtfs, original_poa_gtfs)
