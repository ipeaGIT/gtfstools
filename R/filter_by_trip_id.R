...
filter_by_trip_id2 <- function(gtfs, trip_id, keep = TRUE) {
  gtfs <- assert_and_assign_gtfs_object(gtfs)
  checkmate::assert_character(trip_id, any.missing = FALSE)
  checkmate::assert_logical(keep, len = 1, any.missing = FALSE)
  ...
  gtfs <- filter_routes_from_route_id(gtfs, route_id, `%chin%`)
  gtfs <- filter_fare_rules_from_route_id(gtfs, route_id, `%chin%`)
  ...
  gtfs <- filter_fare_attr_from_fare_id(gtfs, fare_id, `%chin%`)
