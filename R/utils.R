...
#' Test for field existence in a GTFS text file
#'
#' Test for the existence of a given field in a GTFS text file (represented by
#' a column in a \code{data.table}).
#'
#' @param gtfs A GTFS object as created by \code{\link{read_gtfs}}.
#' @param file File in which the field should exist.
#' @param field Field to check existence of.
#'
#' @return \code{TRUE}, if the check is successful, and \code{FALSE} otherwise.
#'
#' @keywords internal
#'
#' @export
test_field_exists <- function(gtfs, file, field) {
  ...
  if (identical(non_existent_field, character(0))) return(TRUE)
...
#' Test for GTFS text file existence
#'
#' Tests for the existence of a given file (as represented by a
#' \code{data.table}) in a GTFS object.
#'
#' @param gtfs A GTFS object as created by \code{\link{read_gtfs}}.
#' @param file File to test the existence of.
#'
#' @return \code{TRUE}, if the check is successful, and \code{FALSE} otherwise.
#'
#' @keywords internal
#'
#' @export
test_file_exists <- function(gtfs, file) {
  ...
  if (identical(non_existent_file, character(0))) return(TRUE)
