#' Return the duration of segments between stops
#'
#' Returns the duration of segments between stops of each specified
#' \code{trip_id}.
#'
#' @param gtfs A GTFS file as read by \code{tidytransit::read_gtfs()}.
#' @param trip_id A string vector including the \code{trip_id}s to have their
#'   segments' duration calculated. If \code{NULL} (the default) the function
#'   calculates the segments' duration for every \code{trip_id} in the GTFS.
#'
#' @return A table containing the duration of the segments of each specified
#'   trip.
#'
#' @export

stop_to_stop_duration <- function(gtfs, trip_id = NULL) {

  # filter gtfs stop_times and frequencies to reduce computation time

  if (! is.null(trip_id)) {
    relevant_trips <- trip_id
  } else {
    relevant_trips <- unique(gtfs$trips$trip_id)
  }

  gtfs$stop_times <- dplyr::filter(gtfs$stop_times, trip_id %in% relevant_trips)

  if (exists("frequencies", gtfs)) {
    gtfs$frequencies <- dplyr::filter(gtfs$frequencies, trip_id %in% relevant_trips)
  }

  # calculate duration in minutes

  gtfs <- tidytransit::set_hms_times(gtfs)

  durations <- gtfs$stop_times %>%
    dplyr::group_by(trip_id) %>%
    dplyr::arrange(stop_sequence, .by_group = TRUE) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(last_stop_departure = dplyr::lag(departure_time_hms)) %>%
    dplyr::filter(stop_sequence > 1) %>%
    dplyr::mutate(
      segment = stop_sequence - 1,
      duration = as.numeric(difftime(arrival_time_hms, last_stop_departure, units = "mins"))
    ) %>%
    dplyr::select(trip_id, segment, duration)

  return(durations)

}
