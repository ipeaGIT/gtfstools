...
#'
#' Filters a GTFS object by `shape_id`s, keeping (or dropping) the entries that
#' relate to such ids in each file.
#'
#' @param gtfs A GTFS object.
#' @param shape_id A character vector. The `shape_id`s used to filter the data.
#' @param keep A logical. Whether the entries related to the passed `shape_id`s
...
#' smaller_gtfs <- filter_shape_id(gtfs, shape_ids)
#' object.size(smaller_gtfs)
#'
#' # drops entries related to passed shape_ids
#' smaller_gtfs <- filter_shape_id(gtfs, shape_ids, keep = FALSE)
#' object.size(smaller_gtfs)
#'
#' @export
filter_shape_id <- function(gtfs, shape_id, keep = TRUE) {
  ...
  if (gtfsio::check_fields_exist(gtfs, "trips", "shape_id")) {

    gtfsio::assert_fields_types(gtfs, "trips", "shape_id", "character")
