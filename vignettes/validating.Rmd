...
we need some GTFS data, which the function accepts in varying format: it can be
...
As we can see, the validation generates a few output files. `report.html`
summarizes the validation results in a nicely formatted HTML page (only
available when using validator v3.1.0 or higher). `report.json` summarizes the
exact same information, but in JSON format, which can be used to easily parse
and process the results. `system_errors.json` summarizes eventual system
erros that may have happened during the validation and may compromise the
results. `validation_stderr.txt` lists the informative messages sent by the
command-line tool to the standard error output stream, which includes the list
of validators in use, eventual error messages, etc. Finally, had the
command-line tool printed anything to the standard output stream, the content
would have been saved in `validation_stdout.txt`[^1].
...
![A snapshot of the HTML report generated using the latest
validator.](../man/figures/html_validation_report.png){width=100%}
